...
UNION ALL SELECT 'ERROR in dProductlines - FactoryKeys don''t exits', gPL.FactoryID, gPL.ProductLineID, '<none>','<none>'					FROM dbo.sx_pf_gProductlines gPL	WHERE gPL.FactoryKey		NOT IN (SELECT DISTINCT dF.FactoryKey FROM dbo.sx_pf_dFactories dF) 
