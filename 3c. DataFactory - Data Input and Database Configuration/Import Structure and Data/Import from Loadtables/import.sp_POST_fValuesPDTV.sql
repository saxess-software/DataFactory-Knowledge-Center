...
2018/06
Import values from load.tfValues with specific load-procedure as source
	EXEC [import].[spPOST_fValuesPDTV] 'SQL','','','','',0,''
*/

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[import].[spPOST_fValuesPDTV]') AND type in (N'P', N'PC'))
DROP PROCEDURE [import].[spPOST_fValuesPDTV]
GO

CREATE PROCEDURE [import].[spPOST_fValuesPDTV]
...
BEGIN
	...
	DECLARE @IsIncrementalValuesFlag			INT
...
-------------------------------------------------------------------------------------------------------------------
GO
GRANT EXECUTE ON OBJECT ::[import].[spPOST_fValuesPDTV] TO pf_PlanningFactoryUser;
GRANT EXECUTE ON OBJECT ::[import].[spPOST_fValuesPDTV] TO pf_PlanningFactoryService;
GO
