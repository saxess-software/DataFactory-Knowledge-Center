...
2018/06
Import rights from loadtable with specific load-procedure as source
	EXEC [import].[spPOST_rRights] 'SQL', '','',''
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[import].[spPOST_rRights]') AND type in (N'P', N'PC'))
DROP PROCEDURE [import].[spPOST_rRights]
GO

CREATE PROCEDURE [import].[spPOST_rRights]
...
BEGIN
	...
	DECLARE	@ReadCommentMandatory		INT 			= ''
	DECLARE @WriteCommentMandatory		INT				= ''
...
-------------------------------------------------------------------------------------------------------------------
GO
GRANT EXECUTE ON OBJECT ::[import].[spPOST_rRights] TO pf_PlanningFactoryUser;
GRANT EXECUTE ON OBJECT ::[import].[spPOST_rRights] TO pf_PlanningFactoryService;
GO
