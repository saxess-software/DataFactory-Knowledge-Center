...
Import User from load.trUser
	EXEC [import].[spPOST_rUser] 'SQL','',''
*/

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[import].[spPOST_rUser]') AND type in (N'P', N'PC'))
DROP PROCEDURE [import].[spPOST_rUser]
GO

CREATE PROCEDURE [import].[spPOST_rUser]
...
BEGIN
	...
	DECLARE @PersonSurname			NVARCHAR(255)
	DECLARE @PersonFirstName		NVARCHAR(255)
	DECLARE @EMail					NVARCHAR(255)
	DECLARE @LDAPIP					NVARCHAR(255)
...
-------------------------------------------------------------------------------------------------------------------
GO
GRANT EXECUTE ON OBJECT ::[import].[spPOST_rUser] TO pf_PlanningFactoryUser;
GRANT EXECUTE ON OBJECT ::[import].[spPOST_rUser] TO pf_PlanningFactoryService;
GO
